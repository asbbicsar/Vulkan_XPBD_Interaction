...
static std::vector<ClothNode> generateClothNodes(uint32_t width, uint32_t height, float spacing) {
    ...
    for (uint32_t y = 0; y < height; y++) {
        for (uint32_t x = 0; x < width; x++) {
            ...
            node.invMass = node.isFixed > 0.5f ? 0.0f : height * width;
...
Ray getMouseRay(GLFWwindow* window, const glm::mat4& view, const glm::mat4& proj, float width, float height) {
    ...
    // 1. NDC(Normalized Device Coordinates) 변환 (-1 to 1)
    float x = (2.0f * (float)xpos) / width - 1.0f;
    float y = (2.0f * (float)ypos) / height - 1.0f; // Vulkan/GLFW는 Y축이 반대

    // 2. 투영 행렬과 뷰 행렬의 역행렬 계산
    glm::mat4 invVP = glm::inverse(proj * view);

    // 3. 근평면(Near)과 원평면(Far)의 점을 월드 좌표로 변환
...
private:
    ...
    void createComputeDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            interactionBufferInfo.buffer = interactionBuffers[i]; // 우리가 만든 Interaction Buffer  
            ...
            descriptorWrites[2].pBufferInfo = &interactionBufferInfo;
    ...
    void drawFrame() {
        ...
        if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
			pc.isPressed = 1;
            Ray ray = getMouseRay(window, ubo.view, ubo.proj, WIDTH, HEIGHT);
            pc.mouseRay = ray;
        }
        else {
			//istate.selectedNodeIndex = -1;
			pc.isPressed = 0;
        }
